...
const extensions = {
  image: ["jpg", "jpeg", "png", "gif", "bmp", "webp", "tif", "tiff", "tga"],
  audio: ["mp3", "wav"],
};
          ...
          >
            ...
            {action.is_error ? (
              ...
              <div className="text-gray-400 text-md flex items-center gap-4">
                ...
                <Select
                  onValueChange={(value) => {
                    // Update the selected format for all files
                    const updatedActions = actions.map((fileAction) => ({
                      ...fileAction,
                      to: value,
                    }));
                    setActions(updatedActions);

                    // Check if all files have a format selected
                    const allFilesSelected = updatedActions.every(
                      (fileAction) => fileAction.to !== null
                    );
                    setIsReady(allFilesSelected);
                  }}
  ...
  return (
    <ReactDropzone
      ...
      onDropRejected={() => {
        ...
        toast({
          variant: "destructive",
          title: "You are Crossing the limit",
          description: "File maximum limit 10",
          duration: 5000,
        });
      }}
      onError={() => {
